...
FROM analytics WHERE min_installs <=50 AND rating IS NOT NULL ORDER BY rating DESC;
...
SELECT SUM(reviews) AS total FROM analytics;

-- Find all the categories that have more than 300 apps in them.
 SELECT category, COUNT(app_name) FROM analytics GROUP BY category HAVING COUNT(app_name) >30
0;
